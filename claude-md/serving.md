...
**Hazard — venv extras + data are hand-provisioned, not deploy-managed.** Bootstrap installs only fastapi+uvicorn into `/srv/qagents/.venv`; anything more (pyarrow 24.0.0 for the qresev bars endpoint, SSM-installed 2026-06-12) is post-bootstrap state. Likewise the OHLCV parquet tree at `/srv/qagents/financial/parquet/` was shipped once through `s3://qagents-artifacts/releases/qresev/` (526 files, 17 MB) — there is **no automatic sync**; refreshing the instance's data means rebuilding and re-extracting that tarball. An § 8.2 instance replacement loses both unless bootstrap grows them — tracked in `data/next-steps/serving.md`.
