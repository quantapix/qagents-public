...
- `~/.aws/config` is metadata only (no credentials) — shape owned by `./templates/aws-config.snippet`, which says **do NOT add a `[default]` block**: with none, an unwrapped command errors `NoCredentials` instead of silently routing somewhere. Both workstations had drifted to a `[default]` carrying `login_session = …:root` (removed 2026-08-06; detail in `serving/runbooks/root-account-access.md` + ns:serving/86) — invisible precisely because every deploy path is wrapped. Every deploy command selects `qagents-deploy` explicitly.
- IAM principal carries TWO scoped customer-managed policies (`./policies/qagents-deploy{,-aux}.json` — main frozen at IAM's 6,144-char cap since the 2026-08-01 split; new grants land in aux). Sid list, rationale + the self-serve version-push runbook (5-version cap, `LimitExceeded` recovery): `data/charters/serving/cloud-base/aws-contract.md` § Policy model → `./runbooks/policy-version-push.md`. **Never** `AmazonS3FullAccess`; never `put-user-policy` against this user — that recreates the retired inline-policy shape (`reference_iam_user_inline_policy_2kb_cap`). **"New grants land in aux" governs NEW grants; a REPAIR to an existing Sid's `Resource`/`Condition` stays in main** — splitting one logical grant across two policies is how the two drift, and the rule's stated rationale is the char cap, which a repair must still respect (live v18 = 6,018 of 6,144). **Read the ARN in the denial message, never the one you wrote:** v18 (2026-08-10) existed because `Ec2SnapshotAccountScoped` granted `ec2:CreateSnapshot` on the account-scoped snapshot ARN while AWS evaluates it against the **account-less** `arn:aws:ec2:<region>::snapshot/*` (snapshots are shareable) — file matched live Sid-for-Sid, so it was never drift, and the sibling `DeleteSnapshot` matched fine, hiding it. Version slots are 5 and full: a push deletes the oldest non-default first.
...
**The roster IS fingerprinted since 2026-08-06 — `parity.sh` `env.remotes`.** Sorted remote-NAME set per qagents-class repo (names, not URLs — URLs carry the per-host `github-qpur` alias and would mint noise between two correct hosts), compared **outside** the same-profile guard on purpose: a remote roster has nothing to do with which venv extras a host requested. Still `git remote` BOTH repos when provisioning a new host: **a new fingerprint field reads `unmeasured` until BOTH hosts have re-published carrying it**, so the gate is blind to this class on any host whose last publish predates the field. Why it was needed (qyel sat 3-of-4 undetected behind an all-green `env.wrappers`): memory `project_serving_subproject`.
...
- **Credential rotation — four rules minted by the 2026-07-30/31 pg loss-and-recovery.** (a) A session rewriting a **host-scoped** secret seat MUST assert `hostname` FIRST (memory `feedback_assert_hostname_before_host_scoped_secret_write`). (b) Verification is a **live probe FROM each seat host** — a cross-host hash compare passes while both copies are burned (`feedback_one_secret_two_at_rest_copies`). (c) `provision-pg-lan.sh` is committed mode 644, so the recipe is `sudo bash <path>`, never `sudo <path>`. (d) The sweep is not the two laptops — **qblk's `~/.pgpass` holds qagents rows too**, and stopping at qred leaves qblk's restore-verify lane failing auth. **(d) NAMES THE WRONG COORDINATE — corrected 2026-08-09.** It points at the client FILE; the sweep unit is **the CLUSTER holding the role**. qblk runs two (`18/main` @5432, `18/verify` @5433), each with its own `qagents` role; the 07-30/31 rotation altered one, and `pg-restore-verify.sh` then failed every weekly run from 2026-07-26 until it was cured (verified dump aged to 321.8 h against a 216 h threshold). A host can be half-rotated, and the missed half is the one no DAILY lane exercises. **A hash compare cannot see this** — both at-rest copies agreed and were correct; the authority is the SERVER role, so only a live probe per port separates them. Remedy: `ALTER ROLE qagents PASSWORD …` in the 5433 cluster only (heredoc — `psql -c` does not interpolate `:'pw'`), then let the verify rotate its own markers. Incident detail: memory `feedback_one_secret_two_at_rest_copies` § 2026-08-09. Incident detail: memory `project_serving_subproject`.
