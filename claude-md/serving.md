...
- [`inventory/<YYYY-MM-DD>/`](./inventory/) — the **verifiable backing** for INVENTORY.md. Exhaustive structural dump of every AWS service + every entry, captured by `serving/scripts/aws-snapshot.sh` running as **root in CloudShell** (deploy-principal MFA-Deny blocks ACM/R53/IAM/KMS/GuardDuty/SNS/Budgets enumeration). Few-hundred-file dated tree per snapshot; `MANIFEST.md` at the root summarizes census + drift + known gaps. Re-run at every phase boundary; the **git diff between two dated snapshots is the canonical change report**, no manual prose drift required.
...
The decisions (OIDC role for CI, build-tarball deploys, Caddy not nginx, no Cloudflare, CDK in TypeScript, single shared EC2, unitary `qagents-deploy` IAM principal, ≥2 virtual MFA devices) and the naming-conventions table are load-bearing. Change them only via a deliberate amendment to `data/specs/serving-2026-05-26.md`; never via a one-off script tweak.
...
The Quantapix-branded diagram primitives + layouts live here, packaged as the `@qagents/diagram-kit` workspace pnpm package. It is the legitimate shared-code seam for cross-subproject diagram primitives + the kit-owned status-hub schema — sibling-of-subprojects, not `serving/`-internal (it lives under `serving/` only because the diagram work was scaffolded here). Anyone (designing/web, explaining/, future consumers) may import it; see § 6.

The kit's display-mode contract — the closed set of `*Emit` schemas the kit ships (Diagram, Table, StatCard, KpiStrip, FilterChipGroup, Dashboard) and what stays consumer-owned (Chart, Graph, 3D-mix, Video, App-shell) — is fixed by `data/specs/display-modes-2026-05-07.md`. Adding a new emit kind requires a kit minor version bump + spec amendment + e2e coverage; v0.4.0 closed the set.

The kit's source-of-truth design bundle lives at `data/renders/serving-design/` (read-only, regen-wholesale). Sync procedure when a new bundle arrives is documented at `serving/diagrams/kit/README.md` § "Sync with the design bundle".

**Consumers must rebuild the kit before their own typecheck/build/dev.** The kit emits to `dist/` (gitignored), and `pnpm install` does not auto-run workspace-package builds. Pointing the kit's `main`/`types` at `src/` directly was tested and rejected: `interactivity.ts` imports DOM types that downstream Node-context tsconfigs don't load. Adopted pattern is `pre-X` hooks in every consumer's `package.json` running `pnpm -F @qagents/diagram-kit build` before the consumer's own step. Wired today on `serving/package.json` (`pretypecheck`) and `designing/web/package.json` (`predev`, `prebuild`, `precheck`); future consumers need the same.

## 5. Diagram prompts are voice-controlled

`serving/diagrams/0X-*.prompt.md` are Claude Design prompts. They enforce Quantapix branding (BracketedQ glyph, approved brand names only), engineer-debugging-a-system voice on labels (no marketing fluff), and the shape-agnostic Node primitive (no AWS-service glyphs). The same `0X-*.prompt.md` shape is used elsewhere for Claude Design prompts that drive other rendering kits: `visualizing/graphs/0X-*.prompt.md` authors the Lean4 proof-graph rendering kit (consumed by `verifying/web/`; migrated from `proving/graphs/` 2026-06-03 — `proving/` is Lean + Python only).

When iterating a Design return on a diagram bundle, lift any richer reading the round surfaced into round N+1's prompt (top-§ thesis + tightened variant specs) rather than treating it as deviation. Pin: `feedback_design_round_richer_than_prompt_promote`.
...
Per-node `details` is an **HTML string** (rendered by the consumer via Astro's `set:html`, not `{expr}` which would escape entities). Keep detail prose factual — services + costs + scope phrasing matching `data/specs/serving-2026-05-26.md`, not marketing fluff.

**Grid-diagram vs. `TableEmit` vs. `DashboardEmit` judgment.** Since v0.3.0 (2026-05-06), naturally tabular content (inventories, principal × policy matrices, trust maps) ships as a `TableEmit` instead of an edgeless `gridLayout` diagram. Since v0.4.0 (2026-05-07), tables that benefit from a header strip + state-aware filtering wrap into a `DashboardEmit` (KpiStrip + FilterChipGroup + the underlying `*-table` TableEmit). Prefer the table when cells carry strings/links/pills and the user reads them row-by-row; promote to dashboard when a state column warrants a kpi summary + filter affordance; keep the diagram when cells need to be visually grouped, hover-pinned, or attached to inspector details.

Edgeless (zero-edge) diagrams must use the kit's `gridLayout`, not `dagreLayout` (dagre collapses them to one rank); see `reference_dagre_grid_for_edgeless`. No serving diagram is edgeless today (`service-inventory` + `security-baseline` ship as `TableEmit`/`DashboardEmit`); the rule still applies to any future matrix-shaped serving diagram.

The summary diagram surfaced on the index card is `static-sites`. Status-pill state machine for serving: `OK` (steady state — every initial stack has shipped); `BUILDING` only if a `cdk deploy` is in flight.
...
- **Single unified IAM principal `qagents-deploy`** — one IAM user, two virtual MFA devices (`Imres-iPhone` primary + `Xmaos-iPhone` backup), one aws-vault profile, one Keychain entry. Locked at `data/specs/serving-2026-05-26.md` § 2 decisions 7 + 8. Blast-radius isolation lives in policy-level MFA-Deny + 1h STS window, not principal multiplication.
...
- IAM principal carries the scoped customer-managed policy at `./policies/qagents-deploy.json` (`arn:aws:iam::072264701492:policy/qagents-deploy`) — full Sid list, coverage (buckets / CF distributions / CodeCommit / CDK day-2 / EC2-SSM-EBS-modify / ManageOwnDeployPolicy / DenyEverythingWithoutMFA), and rationale at `data/specs/serving-2026-05-26.md` § 4.2. **Never** `AmazonS3FullAccess`. Inline-policy form retired — see `reference_iam_user_inline_policy_2kb_cap`.
...
**Deploy script shape — `./scripts/deploy-site.sh <host>`** (full contract at `data/specs/serving-2026-05-26.md` § 5.3):
...
- **Published video deliverables** (MP4 from HeyGen/Remotion, `explaining/`-catalog-class) → `qagents-videos` via `./scripts/upload-video.sh <local> <topic>/<file>.<ext>` → `https://videos.quantapix.com/<topic>/<file>.<ext>`. Public CDN; decoupled from site deploy; immutable-keyed (no CF invalidation).
...
- **Interim/scratch binaries (gitignored, temporary)** — HeyGen takes, Remotion ProRes, Resolve cut renders — mirror to `qagents-clips-backup` via `./scripts/backup-clips.sh <sub> <slug> [<class>...]`. Private; Standard → IA(30d) → Glacier IR(90d) → expire(365d). Operator-fired only. Spec: `data/specs/clips-backup-s3-2026-05-24.md`.
- **Durable version-pinned artifacts (gitignored, kept forever)** — regenerable-today-but-not-durably ground truth keyed by a version-pin (class #1: `uscode` full corpus + USLM XML at PL-119-84; class #2: `parquet` market-data hub — `financial/parquet/` ohlcv + ta-reference + gics, version-pin = ingest date; class #3: `usc-waves` do-axiomatize-uscode blind fan-out archive — `proving/waves/<wave-id>/`, version-pin = the `<wave-id>` itself, producer `proving/scripts/dau.sh --archive-wave`; class #4: `trading-waves` its accounting twin — `accounting/waves/<wave-id>/`, producer `accounting/scripts/dat.sh --archive-wave`, same shape via shared `resolve_wave_class` helper) → `qagents-archive-backup` via `./scripts/archive-artifact.sh <class> <version-pin> {push|verify|restore <dest>}`. Private, **versioned**; current versions rest in Glacier IR at 0d with **no expiration**; noncurrent → Deep Archive(30d) → expire(180d). Content-addressed (sha256 metadata + per-pin MANIFEST.json), one immutable `<class>/<version-pin>/` prefix per pin. Durable inverse of the clips lane; operator-fired only. Spec: `data/specs/artifact-archive-s3-2026-05-29.md`.
...
**Hazard + structural fix — `PROTECTED_PATHS`.** Trade-off of the above: the binary must exist in the *building* worktree's `dist/`, but worktrees don't carry gitignored files. A `pnpm deploy` from a fresh worktree without the binary triggers `s3 rm --recursive` followed by `s3 sync --delete`, wiping the live copy and leaving the page with a 404'd asset. Hit on quantapix.com 2026-05-10 + 2026-05-11 (Janet-preview.mp4). Fix: declare a per-site `PROTECTED_PATHS=("videos/*.mp4" ...)` bash array in `./sites/<host>.env`. `deploy-site.sh` reads the array and threads `--exclude <glob>` through the wipe AND both `s3 sync --delete` passes, so a worktree-without-binary deploy can't take the live URL down. Currently set on `quantapix.com.env`; femfas.net needs none (its `public/*.pdf` legal filings are committed to git per `documenting/CLAUDE.md`). Updates to the binary itself still go via `aws s3 cp` + targeted invalidation — the protection only stops accidental deletion.

**Testing the deploy script — `DEPLOY_DRY_RUN=1`.** Echoes each `aws ...` command as `+ aws ...` instead of executing, and skips the `pnpm build` pre-step. Pinned by `data/specs/serving-2026-05-26-tests/tests/20-deploy-site-protected.sh` (pure bash, no AWS) so a future refactor can't silently lose the `PROTECTED_PATHS` propagation through every mutation, or break the `DEPLOY_NO_WIPE=1` suppression. Run the whole suite with `bash data/specs/serving-2026-05-26-tests/run.sh`. Companion live-URL e2e in `designing/web/tests/e2e/site.spec.ts` (`large media URLs` describe block) HEADs the protected URL post-deploy and asserts 200 + `video/*` content-type + >1MB.
...
- `aws ssm send-command` runs fetch + atomic-swap + `pip install -r requirements.txt` + `systemctl reload <app>.service`
...
Serving/ owns the AWS-touching artifacts of `data/specs/cron-ec2-migration-2026-05-19.md` — CDK deltas, EC2-side scripts + systemd units under `scripts/ec2-cron/`, ship script `scripts/deploy-ec2-cron.sh`, manifest verifier, structural test at `data/specs/serving-2026-05-26-tests/tests/40-ec2-cron.sh`. Operational deploy contract + first-fire smoke: `data/specs/serving-2026-05-26.md` § 6.6 + § 8.3 (SDK-lane resume post-2026-06-15). Managing/ + trading/ + `code/agent_sdk/` own the cross-cutting + out-of-serving-scope files; per-file owner tags in the spec's Appendix A.1/A.2. Same `qagents-app-1` + `qagents-app-role` + `qagents-artifacts` + `alias/qagents-cmk` as § 8.5 — additive only.
...
**Two-clock TZ model.** TZ is pinned in two layers, both load-bearing. (1) Timers FIRE at ET wall-clock via `OnCalendar=… America/New_York` on every `.timer`. (2) The routine *process* is separately pinned to ET via `Environment=TZ=America/New_York` on all three cron `.service` units (`qagents-routine@`, both `qagents-tarball-build-*`). The box TZ stays UTC (AL2023 default; do not change it — FastAPI + Caddy + cw-agent from § 8.5 share the box). Without the process pin a 22:00 ET `overnight-research` fire — the only routine that crosses the UTC day boundary — would compute a next-day date for `research/<date>.md`. Provenance instants stay UTC (`date -u` ignores `$TZ`); `build-tarball.sh` mirrors the inline TZ for hand-run correctness. Spec § 3.4.
...
Two qagents-class repos (`qagents`, `dot.claude` = `~/.claude/`) push to **three** remotes. Public-org GitHub push goes through `git push-quantapix` (see [[reference_quantapix_org_repos]]; 3 predecessor repos retired 2026-05-18, consolidated under `publishing/quantapix/`).
...
`remote.pushDefault = github`, `push.default = current`. Use `git push-all` (executable at `serving/scripts/git-push-all`, symlinked to `~/.local/bin/git-push-all`) to sweep all three in one command — it skips unconfigured remotes silently and wraps the `aws` push in `aws-vault exec qagents-deploy --` automatically. Both git-subcommand wrappers — `git-push-all` and the public-org `git-push-quantapix` (§ source-of-truth `publishing/quantapix/`) — live at `serving/scripts/git-push-{all,quantapix}`, each symlinked onto PATH at `~/.local/bin/`. The repo-wide multi-repo sweeper `scripts/push-all.sh` (root) loops `git push-all` across every qagents-class checkout.
