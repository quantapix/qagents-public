...
- `~/.aws/config` is metadata only (no credentials) — shape owned by `./templates/aws-config.snippet`, which says **do NOT add a `[default]` block**: with none, an unwrapped command errors `NoCredentials` instead of silently routing somewhere. A `[default]` drift is invisible precisely because every deploy path is wrapped (`serving/runbooks/root-account-access.md`). Every deploy command selects `qagents-deploy` explicitly.
...
**Three rules for authoring any cross-host check** live in the `scripts/parity.sh` header (demoted M-V1, 2026-08-09) with their worked instances — read them there before writing or extending any check.
...
- **Mirror-guard:** `bash serving/scripts/harden-mirrors.sh [--verify]` — per-ref `update` hook on both nodes' `qagents.git` + `dot.claude.git` protecting `refs/heads/main` (non-FF + delete refused; `<node>/*` stays deletable for `/pull --tidy`).
