...
- **Exception — YouTube channel art layer.** Channel banner, titles, and thumbnails intentionally depart from calm voice (loud, single-word anchors); scoped to that art layer only — never narration, in-video copy, website copy, or the public README. All YouTube **channel** material is owned by `publishing/`, not here (moved 2026-06-02; path roster: `publishing/CLAUDE.md` § 2); content (scripts/graphics/narration + the Janet master in `voice/`+`avatar/`) stays here. Spec `publishing/youtube/DESIGN-SYSTEM.md` § "Voice override" + memory `feedback_youtube_voice_override`.
...
explaining/
  ...
  scripts/<topic>/<n.m>-<subject-slug>/  (POSITION; one of the 50 — neutral subject id, title is NEVER in the path)
    ...
    phase{0,1,2,3,5,6,8,9}_*.py per-phase drivers (channel-wide; two-lane phase1/phase3 PIP decoration). Shorts are purpose-drafted (`data/specs/explaining-shorts-social-2026-06-15/`; `short/phase9_shorts.py` thin over `scripts/_shorts.py`; `/shorts/` = manual YouTube flow).
...
**Cue/take outputs are NOT durable, and the symlink model is not protection** (corrected 2026-08-08 — the memory above previously asserted that `worktree remove` could not wipe them, which is true of *teardown* and of nothing else). The `videos/*/long/{takes,auditions,cues,cuts,captions}` tree lives at canonical behind a symlink that `.worktree-links` plants in EVERY worktree regardless of subproject, so any session's file operation reaches it and no gate, cron, or producer watches. Live instance: 45 gitignored ProRes cue files removed from canonical on 2026-08-01, detected only by a `rendering/` `--dry-run` five days later; affected episodes + the re-render cost are on `ns:explaining/30`, deleter UNCONFIRMED at `ns:qagents/123`. Treat the S3 backup below and `markers.csv` as the retention, never the working tree. **Since 2026-08-08 there is a second, git-durable path** (R-22): `retire.sh --archive '<lane>'` on a keep-product lane sweeps the UNTRACKED half into the store as `archive.blob` (`class=keep-product-archive`) — keep-product bars DELETION, never ARCHIVAL, so this is not a reap license. `explaining/videos/*` is registered `durability=none` in `lanes.tsv` and qyel's 1.2/3.1 cue trees are the only copies anywhere, so they are the first sweep owed. Cost is gzip-bytea PER FILE, so size the big cohorts deliberately — prefer narrower registry rows over sweeping a whole cohort blind.
...
| 5. Composition + subtitles + loudness + render | DaVinci Resolve Studio | Per-video drivers consuming `resolving/` (wrapper `resolving/davinci/` + skills S1..S10); driver shape + spec per § 4 deliverable shape. |
...
`github.com/quantapix` (drive Promise 1; spec family `data/specs/publishing-2026-05-31/`).
