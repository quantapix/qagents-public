...
The per-framework roster (kernel · specs · source guide · statutory text for all **eleven** goldens — Civil RICO, Title VI, CivilRights, Title IX, Title VII ×3, Rehab §504, Age Act, Title II Enf, ADA, **Terrorism**, **Trafficking**) lives in memory `reference_proving_doctrine_pins` § "Frameworks roster", which also carries the case-law doctrine pins and the top-level inductive shapes. **Those shape names are answer-shaped and are NOT restated here** (§ Golden SHAPES below).
...
Spec: `data/charters/proving/specs/axiomatize-uscode-2026-05-29/SPEC.md`. Extends the hand-built frameworks to a phased, title-by-title program over the Code, produced redundantly by top-tier-model subagent teams along **10 orthogonal axes** (5 core always fanned out + 5 opt-in via `--axes`; roster = spec § 6.1, single owner), then reconciled. Cross-strategy agreement, proved in the kernel via `Bridge.lean` lemmas, is the correctness signal for the LLM→Lean mapping (no other oracle exists).
...
Relocated 2026-08-09 (ns:qagents/146). Read it before ANY wave — it sits in the
wave lane's own skill, which is where every wave starts. Live wave partition
stays `remediation-roster.json`, never prose counts.
...
- **Lean** pinned via `lean-toolchain` — **read the pin from that FILE, never from prose (this bullet included, which is why it names no version).** The lockstep check compares the three axes' files to each other, never to what a CLAUDE.md claims, so a bump verifies green while every prose copy rots. Lockstep with `accounting/` + `studying/` per root CLAUDE.md § Lean4 axes inv-4 — ACCEPTED **and** REJECTED example proofs replayed on every axis at every bump. Sibling pins, the replay recipe (a generated-`Facts`-olean trap that mimics a toolchain break), the per-branch deviation: memory `feedback_lean_toolchain_bump_verify_example_proofs`.
...
- **Open-work reading rules (roster-field definitions · `check_olean_closure` polarity) → `.claude/skills/dau-manual/SKILL.md` § Pre-wave (moved verbatim, spread 2026-08-17 M-P2)**; the exit-code semantics (6 = precondition unmet / could-not-look, 5 and 7 = the real finding) are demoted into `code/lean_tools/attest_axioms.py`'s own header — read them where the tool speaks.
