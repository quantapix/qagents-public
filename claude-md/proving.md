...
These hand-built frameworks are the **golden reference** for the broader program below — automated encodings are scored against them. The six discrimination goldens after CivilRights + the seventh (Title II Enf §2000a-5(b)) all arrived via the **golden-expansion path** (blind Tier-B `gap_*` → hand-built golden supplying the second witness → re-bridge Tier-A sorry-free); all driver-operational + golden-bridged. The **eighth (ADA, 2026-06-24)** is the first **golden-FIRST** build of this batch — hand-built kernel + specs ahead of any blind wave, so its bridging USC wave (§§12112/12132/12182/12203 → `ValidADAComplaint`) is forward work, not a re-bridge. Per-golden derivation + gap + mechanism detail: `predicates/README.md` + close summaries + memory `reference_proving_doctrine_pins`.
...
- **Hierarchical-predicate depth layer (2026-06-20; H0–H4 ALL landed — incl. the first end-to-end decomposed example `examples/usc_t18_enterprise_hier/` and the decompose-by-default scale wave `2026-06-20-t18-1962-h4` (T18 §1962, parity-vs-RICO-golden PASS, promoted `T18.Prohibited.*`); § 8 calibration telemetry wired report→coverage→WAVE; amendment LANDED studying-side).** A depth amendment UNDER the 10 axes: flat opaque predicates decompose into case-law **leaf predicates** (LLM-decided) + kernel-**derived** high-level predicates joined by **procedural axioms/theorems**, discharged by `Proving/USC/Common/Tactic.lean`'s `prove_element` — the LLM extracts leaves, the kernel derives the composite via proof search. **Oracle guard (load-bearing):** search automation (`grind`/`aesop`) NEVER discharges a `Bridge`/`GoldenBridge` agreement lemma (term-only); `score_bridge.py`'s `reject_automation_in_bridge` enforces it syntactically (**exit 4**, ahead of `sorryAx`; test `…/tests/cases/t_11_hier_oracle_guard.sh`, mirrors accounting's `t_17`). Model **stratification**: Tier 0 kernel-decides (zero LLM) / Tier 1 Opus leaf-extraction (default, exit-3; a leaf class downgrades to a cheaper model only after the five-point bake-off gate clears — `scripts/bakeoff_tier1.py`; eligibility is per-leaf-class, NON-MONOTONIC across leaves, never model-wide) / Tier 2 Opus proof-driving. Spec `data/specs/axiomatize-uscode-2026-05-29/hierarchical-predicates-2026-06-20/SPEC.md` (§§ 13/13.1); debates `data/debates/axiomatize-hierarchical-predicates-2026-06-20{,-r02}.md`; H1 cell `Proving/USC/T18/Hier/`. Owed cross-axis amendments listed there.
- **Graph projections (Half-A domain graph).** `scripts/uscode_graph.py --cite-graph` (stdlib, system `python3`, no venv) builds the deterministic Axis-5 §-citation graph (one `Section`/`document` node per operative section keyed by the `usc_cite` join form + Title▸Chapter `cluster`/`cluster_kind` + directed cross-reference `links`), emitting the **`qgraph-wire/1` node-link envelope** (networkx `node_link_data(edges="links")`; `graph.axis="textual"`, `domain="legal"`, `privacy="public-ok"`) consumed by BOTH the graphs-2 `fromNodeLink` reader (mounts in `verifying/`; monitoring pipeline M2) AND `qagents.lean_graph`'s `anchor()` |D| join (reads only `nodes`, so consumer-stable). Default emit = the T18/T42 mount slice (~10k sections); `--all-titles` = the full 62,831-section universe (M2 large-graph path). `python -m qagents.lean_graph --axis textual --domain-graph graphs/structure/graph.json` flips the textual coverage read from the calibration denominator to the live |D|. Also emits the cross-ref centrality table (`--centrality-out`) for the deferred `section_prescreen.py`. `proving/graphs/` is gitignored regenerable ground truth. Tests: `scripts/test_uscode_graph.py`. Spec: `data/specs/axiomatize-uscode-2026-05-29/graph-projections-2026-06-14/SPEC.md` § 4.2.
...
**Sibling parity with `accounting/`.** `accounting/scripts/{lean_parse,lake_trace,report,score_bridge}.py` mirror this pipeline (`lake_trace.py` verbatim; `lean_parse.py`+`report.py` share internals; `score_bridge.py` shares the `#print axioms`/`sorryAx` + R3 oracle-gate shape); `accounting/examples/<id>/graph.json` is byte-compatible with the same `RicoFixture` shape. Lockstep also covers the **S3 push lanes** (`dau.sh`/`dat.sh` `--push-all`/`--push-if-warm`/`aws_session_warm` + `t_08`/`t_09`). Diff-check any patch against the counterpart before landing, and leave the deliberate accounting-side divergences alone (lowercase axiom names, dotted conclusion heads, its own `LociRoster` shape, push-pin source + archive class).
