...
| **Driver** | `scripts/extract_facts.py` | manifest + complaint | `Proving/<Framework>/Facts.lean` (axioms) + audit JSON | `claude -p` invocations (model = `QAGENTS_PREDICATE_MODEL`, default **fable** — the latest top-tier model). |
...
These hand-built frameworks are the **golden reference** for the broader program below — automated encodings are scored against them. All six post-CivilRights goldens (Title IX 2026-06-05; the three Title VII goldens 2026-06-07; Rehab §504 + Age Act 2026-06-08) arrived via the **golden-expansion path**: a blind fan-out bridges to an existing golden only at Tier-B with an *irreducible* `gap_*` (a real statutory element outside the golden's closed shape, e.g. sex/disability/age outside Title VI's `{race,color,nationalOrigin}` enum), establishing the statute as a **sibling** warranting its own hand-built golden (the 10-step workflow below, never folded into the blind fan-out); the same blind cell then re-bridges to the new golden at **Tier-A** sorry-free. All six are **driver-operational + golden-bridged** (specs + ACCEPTED samples + LociRosters — see the table + `predicates/README.md`); per-golden derivation detail (Sandoval-opposite enforcement shape, §2000d-7 edge, carve-outs) lives in the close summaries + memory `reference_proving_doctrine_pins`.
...
- **§ 1962(c) calibration anchor (hand-built, kernel-green):** all 6 axes under `Proving/USC/T18/<Axis>/` + `Bridge.lean` + `Reconciled/T18.lean`; `Bridge.s1962c_tierA` witnesses the spec § 8.3 Tier-A condition; Procedure/Remedy demonstrate the § 5.3 collapse to `Common`. The template cell-agents mirror and the fan-out's scoring reference.
...
Spec roster (authoritative breakdown in `predicates/README.md`): RICO 28, Title VI 17, CivilRights 14, Title IX 21, Rehab §504 19, Age Act 17.
...
**Sibling parity with `accounting/`.** `accounting/scripts/{lean_parse,lake_trace,report}.py` mirror this pipeline; `accounting/examples/<id>/graph.json` is byte-compatible with the same `RicoFixture` shape. `lake_trace.py` is a verbatim copy across the two trees; `lean_parse.py` and `report.py` share internals. The lockstep also covers the **S3 push lanes** (`dau.sh`/`dat.sh` `--push-all` / `--push-if-warm` / `aws_session_warm` + the `t_08`/`t_09` tests). Patches to any of those MUST be diff-checked against the counterpart before landing. Deliberate accounting-side divergences — lowercase axiom names + dotted conclusion heads, a different `LociRoster` shape, the push-pin source and archive class — leave those alone.
...
- **All six post-CivilRights goldens are driver-operational + golden-bridged** (specs + ACCEPTED samples + LociRosters + README rosters; see the frameworks table). Persisting gotchas: the Title IX harassment-severity axiom keeps the upstream spelling `SexualHarassmentSevereePervasive` (spec filename matches); the `{rehab504,ageact}` samples were built with `--stub` (the no-top-tier path). The §2000e family is fully sliced as of the 2026-06-11 Ch21Misc wave (misc tail + §2000d-4a promoted; spec § 10.1). Promotion detail (TitleVIIEnf/TitleVIIDef namespaces, §2000e-12(b) polarity, `Common.Standard` rung `ReasonableCause`) lives in the 2026-06-09 close summary + spec § 10.1.

## Proof-graph visualisation prompts (migrated → `visualizing/graphs/`)

The proof-graph render-design prompts **moved to `visualizing/graphs/` on 2026-06-03** (`data/specs/visualizing-2026-06-03/SPEC.md`; the locked 3-class / 4-edge contract is in that spec § 4, the `Failure` wiring shape at `visualizing/graphs/02-debug-overlay.prompt.md` § "Wiring contract"). `proving/`'s only obligation is the **cross-subproject JSON seam, unchanged**: the audit-JSON / `graph.json` v1 shape from `scripts/extract_facts.py` (schema `data/specs/proving-results-propagation-2026-05-09/SPEC.md` § 2.3) plus that `Failure` shape, consumed by `verifying/web/`'s `/proof-graph/02-debug/` route to distinguish predicate-False vs predicate-Undecided vs kernel-error.
...
**Two-stage, source-first (since 2026-06-04).** The durable fix is upstream: `scripts/redact_complaints.py` redacts the SOURCE `examples/*/complaint.md` (the ground truth predicate sub-agents read) to the **uniform-strict / FBI-director-letter standard** — keep every `X v. Y` legal citation + government bodies; scrub every other personal name, all private medical entities, all residential addresses. Because the source is clean, a fresh `--build` CANNOT re-leak whatever a sub-agent quotes. Covers every example complaint; idempotent and **self-auditing** (case-insensitive DENYLIST + honorific-residue scan). Run it before any fresh derivation; the public court-filing PDFs are NOT a safe text source — `fitz` extracts the text layer *under* graphical redaction boxes (`feedback_graphical_redaction_failure_modes`).
